...
public abstract class AtomDomWriter {
    ...
    public void write(AtomCategory category, Element target) {
        if (category.hasLabel()) {
            target.setAttribute(Atom.LABEL_ATTRIBUTE, category.getLabel());
        }

        if (category.hasScheme()) {
            target.setAttribute(Atom.SCHEME_ATTRIBUTE, category.getScheme());
        }

        if (category.hasTerm()) {
            target.setAttribute(Atom.TERM_ATTRIBUTE, category.getTerm());
        }
    }
    ...
    public void write(AtomContent content, Element target) {
        if (content.hasSrc()) {
            target.setAttribute(Atom.SRC_ATTRIBUTE, content.getSrc());
        }

        if (content.hasType()) {
            target.setAttribute(Atom.TYPE_ATTRIBUTE, content.getType());
        }

        if (content.hasValue()) {
            setTextContent(target, content.getValue());
        }
    }
